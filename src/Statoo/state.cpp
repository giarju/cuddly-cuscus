...
void FSM::fsmAuto(Trajectory currPos)
{
    bool irCondition;
    if (lapanganMerah)
    {
    ...
    {
        float alfa = computeAlpha(passPosition[bola].distance,currPos.distance);
        ...
        //untuk pivot
        if (sqrt(pow((passPosition[bola].distance.x - currPos.distance.x),2) + 
                               pow((passPosition[bola].distance.y - currPos.distance.y),2))<R_ACCEPTANCE)
        {
            kondisi=WAIT;
        }
    ...
    {
        float alfa = computeAlpha(rackTarget[bola].distance,currPos.distance);
        float kecepatan = vwGenerator(rackTarget[bola],currPos,wfStart,6,7,3);
        speedBase->x=kecepatan*cos(alfa);
        speedBase->y=kecepatan*sin(alfa);
        wallFollow();

        if (sqrt(pow((rackTarget[bola].distance.x - currPos.distance.x),2) + 
                               pow((rackTarget[bola].distance.y - currPos.distance.y),2))<R_ACCEPTANCE
                                    && !irCondition)
        {
            kondisi=WAIT;
            bola++;
